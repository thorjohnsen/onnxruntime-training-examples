...
from run_pretraining_ort import BertPretrainingCriterion # , bert_model_with_loss
...
class pretraining_dataset(Dataset):
    ...
    def __init__(self, input_file, max_pred_length):
        ...
        self.input_ids = np.asarray(f["input_ids"][:]).astype(
            np.int64)  # [num_instances x max_seq_length])
        self.input_masks = np.asarray(f["input_mask"][:]).astype(
            np.int64)  # [num_instances x max_seq_length]
        self.segment_ids = np.asarray(f["segment_ids"][:]).astype(
            np.int64)  # [num_instances x max_seq_length]
        self.masked_lm_positions = np.asarray(f["masked_lm_positions"][:]).astype(
            np.int64)  # [num_instances x max_pred_length]
        self.masked_lm_ids = np.asarray(f["masked_lm_ids"][:]).astype(
            np.int64)  # [num_instances x max_pred_length]
        self.next_sentence_labels = np.asarray(
            f["next_sentence_labels"][:]).astype(np.int64)  # [num_instances]
        f.close()
    ...
    def __getitem__(self, index):
        ...
        input_mask = torch.from_numpy(
            self.input_masks[index])  # [max_seq_length]
        segment_ids = torch.from_numpy(
            self.segment_ids[index])  # [max_seq_length]
        masked_lm_positions = torch.from_numpy(
            self.masked_lm_positions[index])  # [max_pred_length]
        masked_lm_ids = torch.from_numpy(
            self.masked_lm_ids[index])  # [max_pred_length]
        next_sentence_labels = torch.from_numpy(
            np.asarray(self.next_sentence_labels[index]))  # [1]
...
def get_args():
    ...
    parser.add_argument("--log_path",
                        ...
                        default="/tmp/dllogger_inference.out",
                        type=str)
...
def main():
    ...
    config.max_prediction_count = args.max_predictions_per_seq * args.eval_batch_size
    ...
    criterion = BertPretrainingCriterion(
        config.vocab_size, args.eval_batch_size, args.max_seq_length)
    ...
    if args.ckpt_dir:
        if args.ckpt_step == -1:
            ...
            model_names = [f for f in os.listdir(
                args.ckpt_dir) if f.endswith(".pt")]
            args.ckpt_step = max(
        ...
        model_file = os.path.join(
            args.ckpt_dir, "ckpt_" + str(args.ckpt_step) + ".pt")
    else:
    ...
    files = [os.path.join(args.input_dir, f) for f in os.listdir(
        args.input_dir) if os.path.isfile(os.path.join(args.input_dir, f)) and 'test' in f]
    files.sort()
    ...
    with torch.no_grad():
        final_loss = 0.0
        final_mlm_acc = 0.0
        mlperf.event(key=mlperf_const.EVAL_START, metadata={'epoch_num': 1})
        for data_file in files:
            dllogger.log(step="Opening ", data={"file": data_file})
            dataset = pretraining_dataset(
                input_file=data_file, max_pred_length=args.max_predictions_per_seq)
            train_sampler = SequentialSampler(dataset)
            datasetloader = DataLoader(
                dataset, sampler=train_sampler, batch_size=args.eval_batch_size, num_workers=1, pin_memory=True)
            for step, batch in enumerate(datasetloader):
                if global_step > max_steps:
                    break
                batch = [t.to(device) for t in batch]
                input_ids, segment_ids, input_mask, masked_lm_labels, next_sentence_labels = batch
                loss, mlm_acc = model(input_ids=input_ids, segment_ids=segment_ids, input_mask=input_mask,
                                masked_lm_labels=masked_lm_labels, next_sentence_labels=next_sentence_labels)
                final_loss += loss.item()
                final_mlm_acc += mlm_acc.item()
                global_step += 1
        ...
        final_mlm_acc /= global_step
        mlperf.event(key=mlperf_const.EVAL_STOP, metadata={'epoch_num': 1})
        mlperf.event(key='eval_samples', value=args.eval_batch_size * global_step)
        mlperf.event(key='eval_accuracy', value=final_mlm_acc, metadata={'epoch_num': 1})
        mlperf.event(key='eval_loss', value=final_loss)
        dllogger.log(step="Loss", data={"final_loss": final_loss})
        dllogger.log(step="Acc", data={"final_mlm_acc": final_mlm_acc})
        with open(os.path.join(os.path.dirname(model_file)) + "acc.csv", "a") as f:
            f.write("{},{},{},{},,,\n".format(os.path.basename(model_file), final_loss, final_mlm_acc, model_file))
    end_infer = time.time()
    dllogger.log(step="Inference perf", data={
                 "inference_sequences_per_second": global_step * args.eval_batch_size / (end_infer - begin_infer)})
